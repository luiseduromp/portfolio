import { EmailTemplate, ContactFormInterface } from '@/components/email/EmailTemplate';
...
export async function POST(request: Request) {
    ...
    if(!token) {
        return Response.json({ success: false, message: "No ReCAPTCHA Token" }, { status: 400 });
    }
    ...
    if(!captchaResponse.success){
        ...
        return Response.json({ success: false, message: "ReCAPTCHA Verify Error" }, { status: 400 });
    }
