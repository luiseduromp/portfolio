  ...
  return (
    <>
      ...
      >
        ...
        <div
          className={cn(
            "border relative border-neutral-00 rounded-lg flex-1 p-2 overflow-y-auto mb-3 bg-neutral-950/50",
            styles.scrollbar,
          )}
        >
...
const ChatBubble = ({
  message,
  ...props
}: { message: ChatMessage } & React.HTMLAttributes<HTMLDivElement>) => {
  const isAssistant = message.role === "assistant";

  return (
    <div
      className={cn(
        "p-2 flex gap-2 items-end w-11/12",
        isAssistant ? "ms-auto" : "",
      )}
      {...props}
    >
      {isAssistant ? (
        <>
          ...
          <div className="size-9 flex-shrink-0 rounded-full bg-neutral-700 flex items-center justify-center">
            <ChatbotIcon className="size-5" />
          </div>
        </>
      ) : (
        <>
          <div className="size-9 flex-shrink-0 rounded-full bg-teal-600 flex items-center justify-center">
            <User size={18} />
          </div>
          <p
            className={cn(
