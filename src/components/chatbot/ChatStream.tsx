...
const AIMessage = ({ message }: { message: ChatMessage }): React.ReactElement => {
    return (
        <div className="p-2 flex flex-col gap-2 items-end w-full ms-auto text-sm">
            <MarkdownRenderer content={message.content} />
        </div>
    );
};

const UserMessage = ({ message }: { message: ChatMessage }): React.ReactElement => {
    return (
        <div className="p-2 flex gap-2 items-center w-11/12 ">
            <div className="size-9 shrink-0 rounded-full bg-teal-600 flex items-center justify-center">
                <User size={18} />
            </div>
            <p className={cn("flex-1 rounded-xl rounded-bl-xs py-3 px-4 bg-teal-600 text-sm")}>
                {message.content}
            </p>
        </div>
    );
