...
import { chatInit } from "@/lib/chatbot";
...
export const ChatStream = () => {
  ...
  const drainQueue = useCallback(() => {
    ...
    setStreamingAnswer((prev) => (prev ?? "") + next);
  ...
  const sendMessage = async (message: string) => {
    ...
    try {
      const fullAnswer = await streamChatMessage(message, messages, {
        onChunk: (chunk) => {
          // Split chunk into individual characters and enqueue
          chunkQueue.current.push(...chunk.split(""));
          if (!isTyping.current) drainQueue();
        },
        onEnd: () => {},
        onError: (error) => {
          console.log("Chatbot message failed", error);
        },
      });

      // Wait for queue to drain before committing the final message
      const waitForDrain = () => {
        if (chunkQueue.current.length > 0 || isTyping.current) {
  ...
  return (
    <>
      <div
        className={cn(
          "fixed p-4 flex flex-col transition-all md:rounded-2xl bg-neutral-900 duration-300 overflow-hidden md:border border-neutral-800",
          "inset-0 md:bottom-6 md:right-6 md:top-auto md:left-auto z-60 gap-2",
          isOpen
            ? "visible opacity-100 w-full h-full md:w-md md:h-130"
            : "invisible opacity-0 w-12 h-12 bottom-0 right-0",
        )}
      >
        <div className="absolute right-4 top-4 flex gap-1 items-center">
          <button
            onClick={handleClear}
            className="text-neutral-500 rounded-full py-1 px-2 text-sm hover:text-white cursor-pointer"
          >
            <RefreshCw size={18} />
          </button>
          <button
        ...
        <div className="mb-3 flex gap-2 items-center">
          <div
            className={cn(
              "size-12 rounded-full flex items-center bg-neutral-700 justify-center",
              isActive ? "opacity-100" : "opacity-80",
            )}
          >
            {isActive ? (
              <ChatbotIcon className="size-7" />
            ) : (
              <ChatbotOutIcon className="size-7" />
            )}
          </div>
          <div>
        ...
        <div
          className={cn(
            "relative rounded-lg flex-1 p-2 overflow-y-auto mb-3 border border-neutral-800",
            styles.scrollbar,
          )}
        >
          ...
          {streamingAnswer && (
            <AIMessage
              message={{ role: "assistant", content: streamingAnswer }}
            />
          )}
          ...
          {isLoading && !streamingAnswer ? (
            <div className={cn("p-2 flex gap-2 items-end justify-end w-full")}>
              <div
                className={cn(
                  "rounded-xl rounded-br-xs py-2 px-3 h-6 bg-neutral-700 text-sm flex items-center justify-center gap-1",
                )}
              >
                <span className="size-1.25 bg-neutral-300 rounded-full animate-bounce [animation-delay:0ms] animation-duration-[0.5s]" />
                <span className="size-1.25 bg-neutral-300 rounded-full animate-bounce [animation-delay:250ms] animation-duration-[0.5s]" />
                <span className="size-1.25 bg-neutral-300 rounded-full animate-bounce [animation-delay:500ms] animation-duration-[0.5s]" />
              </div>
              <div className="size-9 rounded-full bg-neutral-700 flex items-center justify-center">
                <ChatbotIcon className="h-5" />
              </div>
            </div>
          ) : null}
              ...
              {SAMPLE_MESSAGES.map((message, index) => {
                return (
                  <button
                    key={index}
                    className={cn(
                      "block w-3/4 mx-auto rounded-xl p-1 bg-neutral-800 text-sm text-neutral-300 text-center cursor-pointer mb-2 italic",
                    )}
        ...
        <form
          ...
          className={cn(
            "relative flex gap-2 items-center border border-neutral-700 rounded-lg p-2 ps-4 bg-neutral-800",
          )}
        >
          <textarea
            name="message"
          ...
          <button
            className={cn(
              "text-white rounded-sm size-12 flex items-center justify-center bg-linear-to-bl from-sky-400 to-emerald-400 cursor-pointer",
              isActive ? "opacity-100" : "opacity-60 cursor-not-allowed",
            )}
            type="submit"
          >
      ...
      <button
        className={cn(
          "rounded-xl size-12 lg:size-14 flex items-center justify-center cursor-pointer transition-all duration-300 group",
      ...
      >
        ...
        <span
          className={cn(
            "absolute bottom-14 lg:bottom-16 text-sm right-0 font-semibold bg-linear-to-bl from-sky-400 to-emerald-400 w-30 text-neutral-900",
...
const AIMessage = ({
  ...
  return (
    <div className="p-2 flex flex-col gap-2 w-full ms-auto text-sm">
...
const UserMessage = ({
  ...
  return (
    <div className="p-2 flex gap-2 items-center w-11/12 mt-6">
      <div className="size-9 shrink-0 rounded-full bg-teal-600 flex items-center justify-center">
        <User size={18} />
      </div>
      <p
        className={cn(
          "flex-1 rounded-xl rounded-bl-xs py-3 px-4 bg-teal-600 text-sm",
        )}
      >
        {message.content}
      </p>
