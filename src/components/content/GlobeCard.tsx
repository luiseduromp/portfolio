...
export const GlobeCard = ({ lat, lng }: { lat: number, lng: number }) => {
  ...
  useEffect(() => {
    ...
    focusRef.current = [Math.PI - ((lng  * Math.PI) / 180 - Math.PI / 2), (lat * Math.PI) / 180 - Math.PI / 5]
    ...
    <section className="relative">
      ...
      <div className="relative text-center pb-4 lg:pb-0 lg:text-start lg:absolute top-0 left-0 w-full lg:w-1/2 lg:left-1/2 z-20">
        <div className="flex-1 ps-8">
          ...
          <p className="uppercase font-mono text-xl text-teal-400">{date} Open for work</p>
        </div>
      </div>

      <div ref={containerRef} className="w-full aspect-5/2 mx-auto relative overflow-hidden flex items-center">
        <canvas
          ref={canvasRef}
