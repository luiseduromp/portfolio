...
export const LoadingButton: React.FC<LoadingButtonInterface> = ({
  ...
  return (
    <button
      className={cn(
        "bg-teal-500 text-neutral-700 rounded-full px-4 py-2 flex items-center group hover:bg-teal-300",
        "hover:text-black transition-colors duration-200 cursor-pointer",
        "disabled:opacity-50 disabled disabled:bg-teal-300 disabled:text-black disabled:cursor-not-allowed",
        className,
      )}
      {...props}
      disabled={isLoading}
    >
      {isLoading ? (
        <>
          <LoaderCircle className="animate-spin me-2 h-5" /> Sending...
        </>
      ) : (
        <>
