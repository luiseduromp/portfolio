    ...
    <nav ref={navRef} className={cn("", className)}>
      ...
      <div
        className={cn(
          ...
          isOpen
            ? "visible w-70 h-120 md:w-80 lg:w-100 bg-neutral-900/80 backdrop-blur-md pt-16 pb-6  rounded-lg"
            : "invisible size-14",
        )}
      >
        <ul className="overflow-hidden px-8">
          {navLinks.map((link, index) => (
            <li
              key={`link-${index}`}
              className="nav-link mb-2 md:mb-3 lg:mb-4 group"
            >
              <Link
                href={link.url}
                className={cn(
              ...
              >
                <span className="font-mono text-lg md:text-xl lg:text-2xl text-teal-100">
                  0{index + 1}/
                </span>{" "}
                <span className="ps-2 text-2xl md:text-3xl lg:text-4xl font-bold">
                  {link.label}
                </span>
              </Link>
        ...
        <ul className="mt-auto flex justify-center gap-2 flex-wrap px-8">
          {contactLinks.map((link, index) => (
            <li
              key={`cont-${index}`}
              className={cn(isOpen ? "opacity-100" : "opacity-0")}
            >
              <a
                href={link.url}
