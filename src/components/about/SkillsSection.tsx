...
const SkillItem = ({
  ...
  ind,
  ...props
}: { skill: SkillSet; ind: number } & React.HTMLAttributes<HTMLDivElement>) => {
  ...
  useGSAP(() => {
    ...
    gsap
      ...
      .from(
        category,
        { yPercent: 50, opacity: 0, stagger: 0.1, duration: 0.2 },
        "<",
      )
  ...
  return (
    <div
      ref={skillRef}
      key={skill.category}
      className="skill-set md:flex items-center gap-4 py-16 relative overflow-hidden"
      {...props}
    >
      <div className="w-full justify-start md:w-1/2 xl:w-1/3 flex items-center gap-3 mb-4 md:mb-0">
        <h3 className="skill-category font-mono text-2xl text-teal-300 md:text-4xl lg:text-6xl">
          0{ind}/
        </h3>
        <h3 className="skill-category text-2xl md:text-3xl font-bold">
          {skill.category}
        </h3>
      </div>
      <ul className="flex-1 flex flex-wrap">
        {skill.skills.map((skill) => {
          return (
            <li
              key={skill}
              className="skill-item text-lg lg:text-xl px-2 py-1 border-b border-neutral-500 text-neutral-300 hover:text-teal-300 hover:border-teal-300"
            >
              {skill}
            </li>
          );
...
export const SkillsSection = ({
  skillset,
  ...props
}: { skillset: SkillSet[] } & React.HTMLAttributes<HTMLDivElement>) => {
  return (
    <Container {...props}>
