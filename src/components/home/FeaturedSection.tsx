...
const ProjectItem = ({
  project,
}: { project: Project } & React.HTMLAttributes<HTMLElement>) => {
  ...
  useGSAP(() => {
    ...
    const lines = SplitText.create(
      container.querySelectorAll(".project-info"),
      { type: "lines" },
    );
  ...
  return (
    <article
      ref={containerRef}
      className="project lg:flex gap-4 py-12 lg:py-20 items-center"
    >
      <div className="mb-6 lg:mb-0 lg:w-1/3">
        <p className="project-info font-mono text-teal-400 mb-1 text-xl">
          {project.type}
        </p>
        <h5 className="project-info text-4xl sm:text-5xl font-bold mb-2">
          {project.name}
        </h5>
        <p className="project-info text-neutral-400 text-lg mb-3">
          {project.summary}
        </p>
        <p className="project-info font-mono text-teal-400 mb-1 text-xl">
      ...
      <div
        ref={coverRef}
        key={`img-${project.id}`}
        className="lg:flex-1 project-img flex items-center justify-center w-full rounded-xl relative group overflow-hidden"
      >
          ...
          <div className="absolute inset-0 p-4 opacity-0 bg-black/70 group-hover:opacity-100 group-data-hover:opacity-100 transition-opacity duration-200 flex gap-4 justify-center items-center">
            {project.infoUrl && (
              <Link
                ...
                className={cn(
                  "px-1 py-1 size-16 rounded-full text-teal-200 flex gap-2 justify-center items-center text-lg",
